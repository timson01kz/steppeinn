...
export default function Home() {
  ...
  const [activeLocation, setActiveLocation] = useState(almatyMapLocations[0].name);
  ...
  const selectedLocation = useMemo(
    () =>
      almatyMapLocations.find((location) => location.name === activeLocation) ??
      almatyMapLocations[0],
    [activeLocation],
  );
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f6f3ed] text-[#17130f]">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_.72fr]">
        <div>
          <SectionTitle
            description="Click each location to preview the travel context around Almaty. This is still mock data, but the interaction shape is ready."
