...
export default function HotelsPage() {
  ...
  const [activeHotel, setActiveHotel] = useState(catalogHotels[0].slug);

  const filteredHotels = useMemo(() => {
    const result = catalogHotels.filter((hotel) => {
  ...
  }, [amenities, maxPrice, minRating, nearby, search, sort, types]);
        ...
        <div>
          <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold">Mock Almaty stays</h2>
            </div>
            <p className="text-sm font-semibold text-stone-500">
              Database not connected. Results are local mock data.
            </p>
          </div>
